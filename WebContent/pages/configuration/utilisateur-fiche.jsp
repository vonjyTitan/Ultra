...
<%if(((Utilisateur)builder.getEntity()).getEtat()==2){ %>
	<a class="btn btn-success btn-xs" href="login-active?id=<%=((Utilisateur)builder.getEntity()).getIdutilisateur()%>"><i class="fa fa-check"></i>Enabled</a>
		<%
		}
		else 
		{
		%><a class="btn btn-danger btn-xs" href="login-desactive?id=<%=((Utilisateur)builder.getEntity()).getIdutilisateur()%>"><i class="fa fa-trash-o "></i> Disabled</a>
		<%} %>
